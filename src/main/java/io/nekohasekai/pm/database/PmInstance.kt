...
suspend fun TdHandler.gc(instance: PmInstance) {
    ...
    val query =
        MessageRecords.select { messagesForCurrentBot and (MessageRecords.createAt less (DateUtil.yesterday().time / 100)) }
    ...
    for (row in database { query.iterator() }) {
        ...
        when (row[MessageRecords.type]) {
            ...
            MessageRecords.MESSAGE_TYPE_OUTPUT_FORWARDED -> {
                ...
                if (database {
                        MessageRecords.select { messagesForCurrentBot and (MessageRecords.chatId eq chatId) and (MessageRecords.targetId eq messageId) }
                            .firstOrNull()
                    } != null) {
