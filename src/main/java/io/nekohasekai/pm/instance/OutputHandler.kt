...
class OutputHandler(pmInstance: PmInstance) : TdHandler(), PmInstance by pmInstance {
    ...
    suspend fun onNewMessage(userId: Int, chatId: Long, message: TdApi.Message, album: AlbumMessages?) {
        ...
        suspend fun execMessages(targetChat: Long, replyTo: Long) {
            ...
            val messages = if (album == null) {
                ...
                try {
                    ...
                    userCalled(
                        userId,
                        ">> ${sentMessage.text ?: "[ ${sentMessage.content.javaClass.simpleName.substringAfter("Message")} ]"}"
                    )
                ...
                try {
                    ...
                    val messages = sendMessageAlbum(
                        targetChat,
                        replyTo,
                        0L,
                        TdApi.MessageSendOptions(),
                        album.messages.map { it.asInputOrForward }.toTypedArray()
                    ).messages
                    ...
                    if (settings.ignoreDeleteAction) {
                        ...
                        withMarkup(inlineButton {
                            ...
                            dataLine(
                                L.DELETE,
                                dataId,
                                targetChat.asByteArray(),
                                messages.map { it.id }.toLongArray().toByteArray()
                            )
        ...
        if (record == null) {
            ...
            if (useIntegration && getMessageOrNull(chatId, message.replyToMessageId)?.let {
                    getChatMemberOrNull(
                        chatId,
                        it.senderUserId
                    )?.status?.isMember
    ...
    override suspend fun onNewCallbackQuery(
        ...
        database.write {
            ...
            MessageRecords.deleteWhere {
                messagesForCurrentBot and (MessageRecords.chatId eq targetChat) and (MessageRecords.messageId inList listOf(
                    messageId,
                    * targetMessages.toTypedArray()
                ))
            }
