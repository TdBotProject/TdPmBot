...
class InputHandler(pmInstance: PmInstance) : TdHandler(), PmInstance by pmInstance {
    ...
    suspend fun onNewMessage(
        ...
        suspend fun MessageFactory.syncToTarget(): TdApi.Message? {
            ...
            if (integration != null && !integration.paused) {
                ...
                try {
                    ...
                    launcher.findHandler<IntegrationMenu>()
                        .integrationMenu(L, me.id, userBot, admin.toInt(), admin, 0L, false)
        ...
        if (album != null) {
            ...
            userCalled(
                userId,
                "<< ${message.text ?: "[ ${message.content.javaClass.simpleName.substringAfter("Message")} ]"}"
            )
